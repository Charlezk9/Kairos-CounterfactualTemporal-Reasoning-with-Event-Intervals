...
- 生产路径：生产 CLI 在源码中固定 D-004-A 的 stage、revision root、revision 和所有输入/输出文件名，不接受任意路径；仅内部 API 与测试可注入临时根。生产操作从 `/data0/hk_data/kairos-zx` 根 dirfd 逐组件以 `O_DIRECTORY|O_NOFOLLOW` 打开/创建，并在关键操作期间长持有 stage/revision/extracted dirfd；禁止以 `resolve`、`os.walk` 或路径字符串重开替代该语义。
...
- 持久性：发布 completion 前必须 fsync 全部正式 regular files，并对目录树 bottom-up fsync，最后 fsync revision root。completion 私有 temp 在 root dirfd 下 `O_EXCL|O_NOFOLLOW` 创建；link 前后核对 temp/final inode，`os.link` 显式使用同一 src/dst dirfd 与 `follow_symlinks=False`，已有 final 必须 `EEXIST` 失败。内部只得忽略本次持有的精确 temp name+inode；unlink 失败或任何遗留 temp 必须使最终独立 `verify_complete` 失败。任一 fsync 失败不得报告完成。
