...
- `sandbox-audit.md`：智能体 2的只读审批结论；由主智能体写入。
...
checkpoint 使用 `PLANNED`、`APPROVED`、`RUNNING`、`COMPLETE`、`BLOCKED`。阶段只有同时满足以下条件才能标为 `COMPLETE`：预定测试通过、工件可追溯、智能体 2完成事后审计、相关文档更新、Git 提交存在。正式结果不得仅以终端输出或聊天记录作为证据。
